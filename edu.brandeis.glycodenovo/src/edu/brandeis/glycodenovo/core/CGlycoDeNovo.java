...
public class CGlycoDeNovo {
	...
	double mMassAccuracyPPM = 5; // ppm = 1000000*mass_error/exact_mass;
									// mass_error =
									// mMassAccuracyPPM*exact_mass/1000000;
	...
	private int mCurrentPeakIdx = -1; // in matlab it is 0, but it is java and
										// index starts from 0
	...
	public void interpretPeaks(CSpectrum spec) {
		...
		for (int i = 0; i < mPeaks.size(); i++) { // step 2
			...
			if (cPeak.getIntensity() < mIntensityThreshold)
				continue;
			mCurrentTargetMass = cPeak.getMass();
			mCurrentTargetMassLow = cPeak.getMassLow();
			mCurrentTargetMassHigh = cPeak.getMassHigh();
			...
			// If this is the last peak, we should set mTryCIon and
			// mFinalPeak...
			// In this condition, we should not try C Ion and we should tell the
			// interpretApeak(), addCurrentTSSToPool() this is the last peak
			if (mCurrentPeakIdx == mNumPeaks - 1) {
				...
				for (int j = mCurrentTPSuperSetSize - 1; j >= 0; j--) {
					...
					if (bIonMass > avgMass + 0.01) // 0.01 is the mass accuracy,
						...
						mPeaks.get(i).getInferredSuperSets().add(ctss);
						mTryCIon = false;
						break;
					}
				}
	...
	private void appendNLinkedRoot() {
		...
		double dMassBIonNoFuc = CMonosaccharideSet.GlcNAc.sacPermethylated.mass * 2
				- (CMass.CH2 * 2 * mPermethylated + CMass.H2O) * 2 + mFinalPeakCompensation;
	...
	private void addCurrentTSSToPool() {
		// % add mCurrentTopologySuperSetB/B1/B2/C/C1/C2 to the search space for
		// the peaks followed
	...
	private void interpretAPeak() {
		if (mCurrentTargetMass < 438) { // interpret as a mono, hence no heavier
										// than heaviest
		...
		// each branch causes a CH2 loss to the joint monosaccharide when
		// permethylated && obj.mDelta only considers linear structure (i.e.,
		// one branch).;
		mLeafPeak = false;
		for (int i = 0; i < mCurrentTPSuperSetSize; i++) {
			...
			for (int ii = i; ii < mTopologySuperSets.size(); ii++) {
				...
				if (!testAndAddTopologySet(branchMassesLow[1] + branchMassesLow[0],
						branchMassesHigh[1] + branchMassesHigh[0])) 
					break;
	...
	boolean testAndAddTopologySet(double massCompensationLow, double massCompensationHigh) {
		...
		// % Check if the branches together are already too heavy or still too
		// light
		...
		// loop through B2B Delta to find whether this peak can be interpreted
		// as a B2B + mono
		...
		if (mCheckMinus2H) {
			for (int i = 0; i < mDelta.B2B.mass.length; i++) {
				double theoMassLow = mDelta.B2B.mass[i] + massCompensationLow;
				double theoMassHigh = mDelta.B2B.mass[i] + massCompensationHigh;
				if (mCurrentTargetMassHigh + CMass.H2 > theoMassLow
						&& mCurrentTargetMassLow + CMass.H2 < theoMassHigh) {
					idx.add(i);
					flagMinusH.add(2);
				}
			}
		...
		for (int a = 0; a < idx.size(); a++) {
			...
			double theoMassLow = mDelta.B2B.mass[i] + massCompensationLow;
			double theoMassHigh = mDelta.B2B.mass[i] + massCompensationHigh;
			if ((newUnit.mClassID == 2 && !mLeafPeak) || mCompositionCountThreshold[newUnit.mClassID - 1] < 1)
				continue;
			String mType;
			if (mFinalPeak)
				mType = "T";
			...
			insertIntoCurrentTSS(new CTopologySet(mType, mCurrentTargetMass,
					Math.max(theoMassLow, mCurrentTargetMassLow + CMass.H * h),
					Math.min(theoMassHigh, mCurrentTargetMassHigh + CMass.H * h), newUnit, mCurrentBranches, this, h));
		}
		...
		if (mTryCIon && !mFinalPeak) {
			...
			for (int a = 0; a < idx.size(); a++) {
				...
				if ((newUnit.mClassID == 2 && !mLeafPeak) || mCompositionCountThreshold[newUnit.mClassID - 1] < 1)
					continue;
				int h = flagMinusH.get(a);
				insertIntoCurrentTSS(new CTopologySet("C", mCurrentTargetMass,
						Math.max(theoMassLow, mCurrentTargetMassLow + CMass.H * h) - CMass.H2O,
						Math.min(theoMassHigh, mCurrentTargetMassHigh + CMass.H * h) - CMass.H2O, newUnit,
						mCurrentBranches, this, h));
			}
		}
		failed = failed && (idx.size() == 0);

		// if failed and the checkGap condition is true, we checkGap to find
		// whether there might exist a gap mono
		...
		if (failed && mCheckGap) {
			...
			// % @@@@ Here. Need to deal with [massCompensationLow,
			// massCompensationHigh]
			// loop through B2B Delta2 to find whether this peak can be
			// interpreted as a B2B + mono
			// step 6 in the paper Algorithm 1
			...
			for (int a = 0; a < idx.size(); a++) {
				...
				if (newUnit1.mClassID == 2) { // Rule: Fuc can only be a branch,
												// not in a linear substructure.
					continue;
				}
				double theoMassLow = mDelta2.B2B.mass[i] + massCompensationLow;
				double theoMassHigh = mDelta2.B2B.mass[i] + massCompensationHigh;
				...
				int h = flagMinusH.get(a);
				CTopologySet newSet = new CTopologySet(mType, mCurrentTargetMass,
						Math.max(theoMassLow, mCurrentTargetMassLow + CMass.H * h),
						Math.min(theoMassHigh, mCurrentTargetMassHigh + CMass.H * h), newUnit2, newUnit1,
						mCurrentBranches, this, h);
				insertIntoCurrentTSS(newSet);
				for (int b = 0; b < 4; b++) {
					if (mCurrentBranches[b] == null) { // correspond to matlab
														// code
						break;
					}
					insertIntoCurrentTSS(new CTopologySet(mType, mCurrentTargetMass,
							Math.max(theoMassLow, mCurrentTargetMassLow + CMass.H * h),
							Math.min(theoMassHigh, mCurrentTargetMassHigh + CMass.H * h), newUnit2, newUnit1, b,
							mCurrentBranches, this, h));
				}
			}
			...
			if (mTryCIon && !mFinalPeak) {
				...
				for (int a = 0; a < idx.size(); a++) {
					...
					if (newUnit1.mClassID == 2) { // Rule: Fuc can only be a
													// branch, not in a linear
													// substructure.
						continue;
					}
					double theoMassLow = mDelta2.B2C.mass[i] + massCompensationLow;
					...
					int h = flagMinusH.get(a);
					CTopologySet newSet = new CTopologySet("C", mCurrentTargetMass,
							Math.max(theoMassLow, mCurrentTargetMassLow + CMass.H * h) - CMass.H2O,
							Math.min(theoMassHigh, mCurrentTargetMassHigh + CMass.H * h) - CMass.H2O, newUnit2,
							newUnit1, mCurrentBranches, this, h);
					insertIntoCurrentTSS(newSet);
					...
					for (int b = 0; b < 4; b++) {
						...
						insertIntoCurrentTSS(new CTopologySet("C", mCurrentTargetMass,
								Math.max(theoMassLow, mCurrentTargetMassLow + CMass.H * h) - CMass.H2O,
								Math.min(theoMassHigh, mCurrentTargetMassHigh + CMass.H * h) - CMass.H2O, newUnit2,
								newUnit1, b, mCurrentBranches, this, h));
					}
				}
			}
		}
	...
	private void insertIntoCurrentTSS(CTopologySet newSet) {
		// % Add a CTopologySet newSet to a CTopologySuperSet of the same mass
		// and same type
		// % in obj.mTopologySuperSets. Create a new CTopologySuperSet if
		// necessary.
		...
		if (newSet.mType.equals("B") || newSet.mType.equals("T")) {
			...
			if (newSet.mMinusH == 0) {
				if (mCurrentTopologySuperSetB == null) {
					mCurrentTopologySuperSetB = new CTopologySuperSet(newSet.mType, mCurrentTargetMass, this,
							mCurrentPeakIdx);
				}
				...
				if (mCurrentTopologySuperSetB2 == null) {
					mCurrentTopologySuperSetB2 = new CTopologySuperSet(newSet.mType, mCurrentTargetMass, this,
							mCurrentPeakIdx);
			...
			if (newSet.mMinusH == 0) {
				if (mCurrentTopologySuperSetC == null) {
					mCurrentTopologySuperSetC = new CTopologySuperSet(newSet.mType, mCurrentTargetMass, this,
							mCurrentPeakIdx);
				}
				...
				if (mCurrentTopologySuperSetC2 == null) {
					mCurrentTopologySuperSetC2 = new CTopologySuperSet(newSet.mType, mCurrentTargetMass, this,
							mCurrentPeakIdx);
	...
	class Delta2 {
		...
		private Delta2(int numClass) {
			double perMassLoss = 2 * mPermethylated * CMass.CH2; // If no the
																	// root
																	// unit,
																	// lose two
																	// CH2s when
