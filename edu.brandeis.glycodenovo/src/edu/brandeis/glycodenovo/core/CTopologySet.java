...
/**
 * root --- mRootMono branchSet --- mSources topologySet --- mTopologies
 */
...
public class CTopologySet {
	...
	// when interpret a peak there might missing a monosaccharide between peak
	// and mroot
	// 000 - 0 - 0
	// previousPeak - missingmono - mrootmono

	Map<Integer, Integer> mTargetPeaks = new HashMap<>(); // peak index - peak
															// type map, 1=b,
															// 2=c
	...
	CMonosaccharide[] mGapMono = new CMonosaccharide[4]; // gap branch mono if
															// gap exist

	// some constructors with different input
	public CTopologySet(String Type, double MassNormal, double MassLow, double MassHigh, CMonosaccharide RootMono,
			CTopologySuperSet[] Sources, CGlycoDeNovo Reconstroctor) {
		mType = Type;
	...
	public CTopologySet(String Type, double MassNormal, double MassLow, double MassHigh, CMonosaccharide RootMono,
			CMonosaccharide MissingMono, CTopologySuperSet[] Sources, CGlycoDeNovo Reconstroctor) {
		this(Type, MassNormal, MassLow, MassHigh, RootMono, Sources, Reconstroctor);
		mMissingMono = MissingMono;
	}

	public CTopologySet(String Type, double MassNormal, double MassLow, double MassHigh, CMonosaccharide RootMono,
			CTopologySuperSet[] Sources, CGlycoDeNovo Reconstroctor, int MinusH) {
		this(Type, MassNormal, MassLow, MassHigh, RootMono, Sources, Reconstroctor);
		mMinusH = MinusH;
	}

	public CTopologySet(String Type, double MassNormal, double MassLow, double MassHigh, CMonosaccharide RootMono,
			CMonosaccharide MissingMono, CTopologySuperSet[] Sources, CGlycoDeNovo Reconstroctor, int MinusH) {
		this(Type, MassNormal, MassLow, MassHigh, RootMono, Sources, Reconstroctor);
		mMissingMono = MissingMono;
		mMinusH = MinusH;
	}
	...
	public CTopologySet(String Type, double MassNormal, double MassLow, double MassHigh, CMonosaccharide RootMono,
			CMonosaccharide GapMono, int b, CTopologySuperSet[] Sources, CGlycoDeNovo Reconstroctor, int MinusH) {
		this(Type, MassNormal, MassLow, MassHigh, RootMono, Sources, Reconstroctor);
		mGapMono[b] = GapMono;
		mMinusH = MinusH;
	}
	...
	public CTopologySet(String Type, double MassNormal, double MassLow, double MassHigh, CMonosaccharide RootMono,
			CMonosaccharide GapMono, int b, CTopologySuperSet[] Sources, CGlycoDeNovo Reconstroctor) {
		this(Type, MassNormal, MassLow, MassHigh, RootMono, Sources, Reconstroctor);
		mGapMono[b] = GapMono;
	}
	...
	public boolean equals(Object o) {
		...
		if (Double.compare(that.mMassLow, mMassLow) != 0)
			return false;
		if (Double.compare(that.mMassHigh, mMassHigh) != 0)
			return false;
	...
	void reconstructFormulas() {
		...
		if (mLegal) {
			mTopologyFormulas = mTopologyFormulas.subList(0, mCandidateNum); // java
																				// sublist
																				// toIndex
																				// is
																				// exclusive!
		...
		if (sourceIdx > 3 || mSources[sourceIdx] == null) {
			...
			if (branchCount > 0) {
				...
				for (int i = 0; i < branchCount; i++) {
					...
					if (mMissingMono == null) {
						if (!CGlycoDeNovo.cLegalGlycosidicBonds[mBranchTopologies[i].mRootMonoClassID
								- 1][mRootMono.mClassID - 1])
							return;
					} else {
						if (!CGlycoDeNovo.cLegalGlycosidicBonds[mBranchTopologies[i].mRootMonoClassID
								- 1][mMissingMono.mClassID - 1])
							return;
					}
					...
					if (mGapMono[i] == null) {
						...
						gwaBranchFormulas[i] = "--4b1D-" + mGapMono[i].mClass + ",p--4b1D-"
								+ mBranchTopologies[i].mGWAFormula + ",p";
			...
			mTopologies.add(new CTopology(formula, gwaFormula, mRootMono.mClassID, peaks, (double) peaks.size(), mass,
